...
export default function Calculator() {
  ...
  const convertToMetric = (value: string, eventName: string) => {
    ...
    if (
      distanceEvents.includes(eventName) ||
      heightEvents.includes(eventName)
    ) {
  ...
  const getPlaceholder = (eventName: string) => {
    ...
    return isMetric
      ? metricPlaceholders[eventName]
      : imperialPlaceholders[eventName];
  };
  ...
  const savePerformanceLocal = () => {
    ...
    try {
      const performance = {
        id: editingPerformanceId || generateId(),
      ...
      if (editingPerformanceId) {
        ...
        savedPerformance = updatePerformance(
        ...
        setEditingPerformanceId(savedPerformance.id);
      }
      ...
      if (newAchievements.length > 0) {
        ...
        toast({
          ...
          description: `You earned ${newAchievements.length} new achievement${
  ...
  const updateResult = (
    ...
    if (field === "result") {
      ...
      if (points > 0 && selectedEventType) {
        const event = eventResults[index];
        const estimatedResult = estimateResult(
          selectedEventType,
          event.name,
          points,
          event.type
  ...
  const hasMultipleDays = () => {
    return (
      selectedEventType === "decathlon" || selectedEventType === "heptathlon"
    );
  };

  const getPerformanceDayTotal = (
    performance: StoredPerformance,
    day: number
  ) => {
    if (!performance.eventResults || !Array.isArray(performance.eventResults))
      return 0;
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-white/90 text-xs mt-0.5 font-medium tracking-wide">
                  A Track & Field Multi event calculator
                </p>
              </div>
            </div>
            ...
            <Card className="glass-card">
              ...
              <CardContent className="p-4">
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {(Object.keys(eventConfigs) as EventType[]).map(
                    (eventType) => (
                      <Button
                        key={eventType}
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          <div
                            className={`p-2 glass-icon-container ${
                              selectedEventType === eventType
                          ...
                          <div className="text-left">
                            <h3 className="font-semibold text-base">
                              {eventConfigs[eventType].name}
                            </h3>
                            <p className="text-xs font-medium">
                              {eventConfigs[eventType].events.length} Events
                            </p>
                          </div>
                        </div>
                      </Button>
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-2 font-medium">
                        Enter your results to auto-calculate points, or enter
                        points to estimate results. Both fields update each
                        other automatically.
                      </p>
                    </div>
                ...
                <CardContent className="p-4">
                        ...
                        [1, 2].map((day) => (
                          <div key={day} className="space-y-3">
                            ...
                            {getEventsByDay(day).map((event, index) => {
                              const originalIndex = eventResults.findIndex(
                                (e) => e.name === event.name
                              );
                              return (
                                <div
                                  key={originalIndex}
                                  className="glass-event-card p-3"
                                >
                                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 items-center">
                                    <div className="flex items-center space-x-2">
                                      <div
                                        className={`p-2 glass-card transition-all duration-200 ${
                                          event.type === "time"
                                      ...
                                      <div>
                                        ...
                                        <p className="text-xs text-muted-foreground font-medium tracking-wide">
                                          ...
                                          ({getEventUnit(event.name)})
                                        </p>
                                      </div>
                                    </div>
                                    <div className="space-y-1">
                                      <Label
                                        htmlFor={`result-${originalIndex}`}
                            ...
                            <div className="glass-card bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-400/30 p-3 mt-3">
                              <div className="flex items-center justify-between">
                                ...
                                <span className="text-lg font-black text-white">
                                  {getDaySubtotal(day).toLocaleString()}
                                </span>
                              </div>
                            </div>
                          </div>
                                  ...
                                  {!isMetric &&
                                    ...
                                    event.result && (
                                      <span className="text-muted-foreground/60 font-normal">
                                        (
                                        {(
                                          parseFloat(event.result) * 0.3048
                                        ).toFixed(2)}
                                        m)
                                      </span>
                                    )}
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <div className="glass-card bg-gradient-to-r from-orange-500/20 to-red-500/20 border-orange-400/30 p-4">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-black text-white">
                              {getDaySubtotal(1).toLocaleString()}
                            </div>
                          </div>
                        </div>
                        <div className="glass-card bg-gradient-to-r from-red-500/20 to-pink-500/20 border-red-400/30 p-4">
                          <div className="text-center">
                            ...
                            <div className="text-2xl font-black text-white">
                              {getDaySubtotal(2).toLocaleString()}
                            </div>
                          </div>
                        </div>
                  ...
                  <div className="flex justify-center space-x-3">
                    <Button
                      ...
                      disabled={
                        totalScore === 0 || !selectedEventType || isLoading
                      }
                    ...
                    <Button
                      onClick={clearAll}
                      className="glass-button bg-red-500/20 text-red-200 hover:bg-red-500/30 border-red-400/30 transition-all duration-200 text-sm px-4 py-2"
                    >
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    >
                      ...
                      <SelectContent className="glass-dropdown">
                        <SelectItem value="all" className="glass-dropdown-item">
                          All Events
                        </SelectItem>
                        <SelectItem
                          value="decathlon"
              ...
              <CardContent className="p-4">
                {isLoading ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground font-semibold tracking-wide text-sm">
                      No Performances Recorded Yet
                    </p>
                    <p className="text-xs text-muted-foreground mt-1 font-medium">
                      Start by selecting an event type and entering results
                    </p>
                  </div>
                ) : (
                  <div className="overflow-x-auto">
                    <Table className="glass-table">
                      <TableHeader>
                        <TableRow>
                          <TableHead className="text-foreground font-black tracking-wide">
                            Date
                          </TableHead>
                          <TableHead className="text-foreground font-black tracking-wide">
                            Label
                          ...
                          .map((performance) => (
                            ...
                            >
                              ...
                              <TableCell className="font-black text-white text-sm py-2">
                                {performance.eventType === "decathlon" ||
                                performance.eventType === "heptathlon" ? (
                                  getPerformanceDayTotal(
                                    performance,
                                    1
                                  ).toLocaleString()
                                ) : (
                ...
                <CardContent className="p-4">
                  <div className="space-y-4">
                    {performances.map((performance, index) => {
                      const radarData =
                        performance.eventResults?.map((event) => ({
                          event: event.name,
                          points: event.points,
