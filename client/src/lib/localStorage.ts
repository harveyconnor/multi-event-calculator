...
export function savePerformance(
  performance: Omit<StoredPerformance, "id" | "date">
): StoredPerformance {
...
export function updatePerformance(
  id: string,
  updates: Partial<StoredPerformance>
...
export function saveAchievement(
  achievement: Omit<StoredAchievement, "id" | "unlockedAt">
): StoredAchievement {
...
export function checkAchievements(userId: number = 1): StoredAchievement[] {
  ...
  if (
    ...
    !existingAchievements.find((a) => a.type === "first_performance")
  ) {
  ...
  if (latestPerformance) {
    ...
    if (
      score >= 7000 &&
      !existingAchievements.find((a) => a.type === "score_7000")
    ) {
      const achievement = saveAchievement({
        type: "score_7000",
        title: "Rising Star",
    ...
    if (
      score >= 8000 &&
      !existingAchievements.find((a) => a.type === "score_8000")
    ) {
      const achievement = saveAchievement({
        type: "score_8000",
  ...
  eventTypes.forEach((eventType) => {
    ...
    if (
      eventPerformances.length >= 3 &&
      !existingAchievements.find((a) => a.type === `${eventType}_specialist`)
    ) {
      const achievement = saveAchievement({
        type: `${eventType}_specialist`,
        title: `${
